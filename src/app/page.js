...
export default function Home() {
  ...
  return (
    <>
      ...
      <main style={{ backgroundColor: '#FFFFFF', minHeight: '100vh', margin: 0, padding: 0, fontFamily: 'sans-serif', overflowX: 'hidden' }}>      
        <nav style={{ backgroundColor: '#FFFFFF', padding: '5px clamp(15px, 5vw, 40px)', position: 'sticky', top: 0, zIndex: 1000, boxShadow: '0 4px 20px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', minHeight: '70px' }}>
          <div style={{ display: 'flex', width: '100%', alignItems: 'center', justifyContent: 'space-between', gap: '15px' }}>
            <div onClick={() => typeof window !== 'undefined' && window.scrollTo({top: 0, behavior: 'smooth'})} style={{ flex: '1 1 auto', display: 'flex', justifyContent: 'flex-start', cursor: 'pointer' }}>
              <img src="/Logocompleto.png" alt="Green Landscaping and Irrigation LLC" style={{ height: 'auto', width: 'clamp(180px, 25vw, 280px)', maxHeight: '90px', objectFit: 'contain' }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'center', flexShrink: 0 }}>
              <div style={{ textAlign: 'right', marginBottom: '4px' }}>
                <p style={{ fontSize: 'clamp(8px, 1vw, 10px)', color: '#6B7280', fontWeight: 'bold', margin: 0, textTransform: 'uppercase' }}>Free Estimate</p>
              ...
              <a href="tel:+17622440708" style={{ backgroundColor: '#FFB703', color: '#0B2219', padding: '6px 16px', borderRadius: '8px', fontWeight: '900', fontSize: '12px', textDecoration: 'none', boxShadow: '0 4px 12px rgba(255,183,3,0.3)', whiteSpace: 'nowrap' }}>CALL NOW</a>
            </div>
          </div>
        </nav>
        ...
        <section style={{ padding: 'clamp(10px, 3vw, 20px)', backgroundColor: '#F9FAFB' }}>
          <div style={{ position: 'relative', maxWidth: '1400px', margin: '0 auto', minHeight: 'clamp(50vh, 65vh, 80vh)', borderRadius: 'clamp(20px, 5vw, 40px)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'clamp(4px, 1vw, 8px) solid #FFFFFF', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
            <img src="/premium-garden.png" alt="Garden Background" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
            ...
            <div style={{ position: 'relative', zIndex: 20, textAlign: 'center', padding: '40px 20px' }}>
              <h1 style={{ color: '#FFFFFF', fontSize: 'clamp(1.8rem, 6vw, 3.5rem)', lineHeight: '1.2', marginBottom: '20px', fontWeight: '900' }}>
                Where <span style={{ color: '#FFB703' }}>Memories</span> Grow <br/> and Lawns Flourish.
              </h1>
              <p style={{ color: '#FFFFFF', fontSize: 'clamp(14px, 3vw, 18px)', marginBottom: '30px', maxWidth: '600px', margin: '0 auto 30px' }}>Your home is your sanctuary. We provide the expert care your landscape deserves.</p>
              <div style={{ display: 'flex', gap: '15px', justifyContent: 'center', flexWrap: 'wrap' }}>
                <button onClick={() => scrollToSection('estimate-section')} style={{ backgroundColor: '#FFB703', color: '#0B2219', padding: '15px 30px', borderRadius: '12px', fontWeight: '900', border: 'none', cursor: 'pointer', minWidth: '160px' }}>GET ESTIMATE</button>
                <button onClick={() => scrollToSection('projects-section')} style={{ backgroundColor: 'rgba(255,255,255,0.1)', color: '#FFFFFF', padding: '15px 30px', borderRadius: '12px', border: '2px solid #FFFFFF', fontWeight: '900', cursor: 'pointer', minWidth: '160px' }}>OUR PROJECTS</button>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="estimate-section" style={{ padding: 'clamp(40px, 8vw, 80px) 20px', backgroundColor: '#FFFFFF' }}>
          <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', gap: '40px', alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ flex: '1', minWidth: '300px' }}>
              <h2 style={{ fontSize: 'clamp(30px, 5vw, 48px)', fontWeight: '900', color: '#0B2219', lineHeight: '1.2', marginBottom: '25px' }}>Ready to Transform <br/> <span style={{ color: '#FFB703' }}>Your Landscape?</span></h2>
              <p style={{ fontSize: '17px', color: '#4B5563', lineHeight: '1.6', marginBottom: '30px' }}>Tell us about your project. Our experts are ready to help.</p>
            ...
            <div style={{ flex: '1', width: '100%', maxWidth: '500px', backgroundColor: '#F9FAFB', padding: 'clamp(20px, 5vw, 40px)', borderRadius: '30px', boxShadow: '0 20px 40px rgba(0,0,0,0.05)', border: '1px solid #E5E7EB' }}>
              {submitted ? <div style={{textAlign: 'center', padding: '40px'}}><h3 style={{color: '#0B2219', fontWeight: '900'}}>Thank you!</h3><p>We have received your request and will contact you soon.</p></div> : (
          ...
          <div className="split-section" style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '40px' }}>
            <div>
              ...
              <div style={reviewFormWrapperStyle}>
                ...
                {reviewSubmitted ? <p style={{ color: '#059669', fontWeight: 'bold' }}>Thanks for your feedback!</p> : (
      ...
      {/* FOOTER */}
      <footer style={{ backgroundColor: '#0B2219', color: '#FFFFFF', padding: '30px 20px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div className="footer-container" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '20px', marginBottom: '30px' }}>
            <div style={{ flex: '0 0 auto', margin: '0 auto 0 0' }}>
              <img src="/Logocompleto.png" alt="Logo" style={{ height: 'auto', width: '130px', filter: 'brightness(0) invert(1)' }} />
            </div>
            <div className="footer-data-container" style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 'clamp(15px, 3vw, 50px)', alignItems: 'center', justifyContent: 'flex-end', flex: '1' }}>
              ...
              <div style={{ minWidth: '140px' }}>
                ...
                <a href="tel:+17622440708" style={{ color: '#FFFFFF', textDecoration: 'none', fontSize: '16px', fontWeight: '900' }}>+1 (762) 244-0708</a>
              </div>
              <div style={{ minWidth: '150px' }}>
                <h4 style={{ color: '#FFB703', margin: '0', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '1px' }}>Email</h4>
                <a href="mailto:info@greenlandgl.com" style={{ color: '#FFFFFF', textDecoration: 'none', fontSize: '14px', fontWeight: '500' }}>info@greenlandgl.com</a>
              </div>
            </div>
          </div>
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '20px', fontSize: '10px', color: '#9CA3AF', textAlign: 'center' }}>
            © {mounted ? currentYear : '2026'} Green Landscaping & Irrigation LLC. All rights reserved
          </div>
        </div>
      </footer>
    </>
